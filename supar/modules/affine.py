...
class SmallBiaffine(nn.Module):
    ...
    def forward(self, x, y):
        r"""
        Args:
            x (torch.Tensor): ``[seq_len, n_in]``.
            y (torch.Tensor): ``[seq_len, n_in]``.

        Returns:
            ~torch.Tensor:
                A scoring tensor of shape ``[n_out, seq_len, seq_len]``.
                If ``n_out=1``, the dimension for ``n_out`` will be squeezed automatically.
        """
        ...
        if self.bias_y:
            y = torch.cat((y, torch.ones_like(y[..., :1])), -1)
        # [batch_size, n_out, seq_len, seq_len]
        s = torch.einsum('xi,oij,xj->ox', x, self.weight, y)
        # remove dim 1 if n_out == 1
        s = s.squeeze(0)
