...
class VISrlModel(nn.Module):
    ...
    def viterbi_decode_BE(self, s_edge, s_label, strans, trans, mask, mask2, B_idxs, I_idxs, pair_dict, prd_idx):
        '''
        for schema BE
        '''
        ...
        # [batch_size, seq_len]
        pred_mask = self.detect_conflict(label_preds, pred_mask, B_idxs, I_idxs, pair_dict, prd_idx)
    ...
    def fix_label_cft_BE(self, label_preds, B_idxs, I_idxs, prd_idx, pair_dict, p_label):
        '''
        solve label conflicts such as B-a and I-b
        '''
        # [batch_size, seq_len]
    ...
    def detect_conflict(self, label_preds, pred_mask, B_idxs, I_idxs, pair_dict, prd_idx):
        """to detect whether exist conflict (now just B-I-I, not consider B_a-Ib)
            for schema BE
        Args:
            label_preds ([type]): [batch_size, seq_len, seq_len]
    ...
    def detect_conflict_BII(self, label_preds, pred_mask, B_idxs, I_idxs, prd_idx):
        """to detect whether exist conflict (now just B-I-I, not consider B_a-Ib)
            for schema BII
        Args:
            label_preds ([type]): [batch_size, seq_len, seq_len]
    ...
    def viterbi_decode_BII(self, s_edge, s_label, strans, trans, mask, mask2, B_idxs, I_idxs, prd_idx):
        '''
        for schema BII
        '''
        edge_preds = s_edge.ge(0.5).long()
        ...
        # [batch_size, seq_len]
        pred_mask = self.detect_conflict_BII(label_preds, pred_mask, B_idxs, I_idxs, prd_idx)
        k = pred_mask.sum().item()  # num of the conflict predicate
    ...
    def viterbi_decode_BIES(self, s_edge, s_label, strans, trans, mask, mask2, B_idxs, I_idxs, E_idxs, S_idxs, prd_idx):
        '''
        for schema BIES
        '''
        edge_preds = s_edge.ge(0.5).long()
        ...
        # [batch_size, seq_len]
        pred_mask = self.detect_conflict_BIES(label_preds, pred_mask, B_idxs, I_idxs, E_idxs, S_idxs, prd_idx)
        k = pred_mask.sum().item()  # num of the conflict predicate
    ...
    def detect_conflict_BIES(self, label_preds, pred_mask, B_idxs, I_idxs, E_idxs, S_idxs, prd_idx):
        '''
        to detect whether exist conflict (now just B-I-I, not consider B_a-Ib)
            for schema BIES
        Args:
        ...
        for i in range(k):
            ...
            while j<length:
                if seq[j] == -1:
                    j += 1
                elif seq[j] in I_idxs or seq[j] in E_idxs:
                    flag = 1
                    break
                elif seq[j] in S_idxs:
                    j += 1
                    ...
                    # seq[j] in B_idxs
                    j += 1
                    if j>=length or seq[j] == -1 or seq[j] in S_idxs or seq[j] in B_idxs:
                        flag = 1
                        break
                    if seq[j] in E_idxs:
                        j += 1
                        ...
                        # seq[j] in I_idxs
                        while j<length and seq[j] in I_idxs:
                            j += 1
                        if j == length:
                            flag = 1
                            break
