...
class Dataset(torch.utils.data.Dataset):
    ...
    def collate_fn(self, batch):
        if not hasattr(self, 'fields'):
            raise RuntimeError(
                "The fields are not numericalized yet. Please build the dataset first."
            )
    ...
    def build(self, batch_size, n_buckets=1, shuffle=False, distributed=False):
        ...
        self.buckets = dict(
            zip(*kmeans(
                [len(s.transformed[self.fields[0].name])
                 for s in self], n_buckets)))
...
class DataLoader(torch.utils.data.DataLoader):
    ...
    def __iter__(self):
        for batch in super().__iter__():
            yield namedtuple('Batch',
                             (f.name for f in batch.keys()
                              ))(*[f.compose(d) for f, d in batch.items()])
