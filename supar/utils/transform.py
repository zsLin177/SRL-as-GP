...
class CoNLL(Transform):
    ...
    def get_spans(cls, sequence):
        ...
        # seq_len contain root
        # results = [[[None]*(len(sequence)+1) for i in range(len(sequence)+1)] for j in range(len(sequence)+1)]
        results = [None] * (len(sequence)+1)
        ...
        for i in range(len(results)):
            if(results[i] == None):
                results[i] = [[None] * (len(sequence)+1) for _ in range(len(sequence)+1)]
        return results
  
    @classmethod
    def get_span_one_sentence(cls, relas, prd_idx):
        span = [[None] * (len(relas)+1) for _ in range(len(relas)+1)] 
        i = 0
        while (i < len(relas)):
            rel = relas[i]
            # print(i)
