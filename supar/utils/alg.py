...
def cky(scores, mask):
    ...
    for w in range(1, seq_len):
        n = seq_len - w
        starts = p_c.new_tensor(range(n)).unsqueeze(0)
        ...
        # [batch_size, n]
        s_c, p = s_c.max(-1)
        s.diagonal(w).copy_(s_c + s_l)
        p_c.diagonal(w).copy_(p + starts + 1)

    def backtrack(p_c, p_l, i, j):
        if j == i + 1:
            return [(i, j, p_l[i][j])]
        split, label = p_c[i][j], p_l[i][j]
        ltree = backtrack(p_c, p_l, i, split)
        rtree = backtrack(p_c, p_l, split, j)
        return [(i, j, label)] + ltree + rtree

    p_c = p_c.permute(2, 0, 1).tolist()
    p_l = p_l.permute(2, 0, 1).tolist()
    trees = [backtrack(p_c[i], p_l[i], 0, length) for i, length in enumerate(lens.tolist())]
