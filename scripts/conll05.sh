#!/bin/bash
# The downloading part is from https://github.com/yzhangcs/crfsrl/blob/main/scripts/conll05.sh.
...
# generate conllu format data in BES schema
for dataset in train dev test brown; do
  echo "Converting ${CONLL05}/$dataset.prop to conllu format in BES"
  python scripts/preprocess.py --prop ${CONLL05}/$dataset.prop --file ${CONLL05}/$dataset-BES.conllu
done